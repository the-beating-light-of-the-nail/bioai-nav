...
// sitemap 与预渲染种子同一数据源；/search 是 noindex 页，不进 sitemap。
// 注意：这里直接静态导入 data/generated/all-resources.json（split-resources.mjs 产出），
...
export default defineEventHandler((event) => {
  ...
  const urls: { loc: string; priority: string; changefreq: string }[] = [
    ...
    { loc: '/tags', priority: '0.4', changefreq: 'weekly' },
  ]

  for (const c of CATEGORIES) {
    urls.push({ loc: `/${c.slug}`, priority: '0.9', changefreq: 'weekly' })
    ...
    for (const [sub, n] of counts) {
      if (n >= SUBCAT_PAGE_MIN) urls.push({ loc: `/${c.slug}/${sub}`, priority: '0.7', changefreq: 'weekly' })
    }
  }

  for (const r of all) {
    urls.push({ loc: `/${r.category}/${r.slug}`, priority: '0.6', changefreq: 'monthly' })
  }
  ...
  for (const [t, n] of tagCounts) {
    if (n >= TAG_PAGE_MIN) urls.push({ loc: `/tags/${t}`, priority: '0.4', changefreq: 'weekly' })
  }
